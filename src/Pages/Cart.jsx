...
const Cart = () => {
  ...
  return (
    <div className="mt-40 sm:mt-28 text-black">
      ...
      <h1> My Cart{" "}({items.length})   <button  onClick={()=> emptyCart(items)} className="ms-2 text-sm text-red-500 Quicksand"><i className="fas fa-trash text-red-500 text-sm"></i>clear cart</button></h1>
      <h1>Total: {cartTotal} </h1>
        ...
        <div>
          {items.map((item, id) => (
            <div key={id} className="flex    justify-center Quicksand  hover cursor-pointer">
